...
description: >-
  synchronized关键字用于保障原子性、有序性。基于对象监视器实现的锁，Java中每个对象都与一个监视器相关联，一个线程可以锁定或解锁，一次只有一个线程可以锁定监视器，试图锁定该监视器的其它线程都会被阻塞，直到它们可以获得该监视器上的锁定为止。
---

synchronized（悲观锁又叫独享锁） 用于保障原子性、有序性。synchronized 基于对象监视器实现的，Java中每个对象都与一个监视器相关联，一个线程可以锁定或解锁，一次只有一个线程可以锁定监视器，试图锁定该监视器的其它线程都会被阻塞，直到它们可以获得该监视器上的锁定为止。
